...
void Lista::Move(vector<int> V){
        
    for(int i=1; i<M_VC[0].GetN();i++){
        
        iter_swap(M_VC.begin()+i , M_VC.begin()+V[i]);
        
        /*
        if (i != V[i]) {
            swap(M_VC[i], M_VC[V[i]]);
        }
        */
    }
}

void Lista::ShowTag(int rk)
{
    cout<<"rank: "<<rk<<"    ";
    for(int i=0; i<M_NN;i++ ){
        cout<<M_VC[i].GetN()<<" ";
...
void GA::SwapBxW(vector<int> B){
    int w=Worst();
    GetList_i(w).Move(B);
    M_Dist[w]=GetList_i(w).Dist_N();
}
...
void scrivi(int n, string a)
{
    ...
    for(int i=0;i<n;i++){
        file_out<<rnd.Rannyu(0,5)<<" "<<rnd.Rannyu(0,5)<<endl;
    }
    file_out.close();
}
