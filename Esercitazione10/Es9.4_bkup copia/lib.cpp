     ...
     for (size_t i = 0; i < l1.GetN() - 1; ++i) {
         ...
         if (has_duplicates) {
           ...
           //raddoppio
             ...
             vector<City> v_prova2;
             
             int taglia2=2+rr*(l1.GetN()-2);

             if(taglia2>1){
                 for(int i=0;i<taglia2;i++){
                     v_prova2.push_back(l1.GetList()[i]);
                 }
             }else{
                 cout<<endl<<"Errore nella funzione Figlia"<<endl<<endl;
                 return 0;
             ...
             for(int i=taglia2;i<l1.GetN();i++){
                 v_prova2.push_back(l2.GetList()[i]);
             }
               
             Lista prova2(v_prova2);
          
             bool has_duplicates2 = false;
