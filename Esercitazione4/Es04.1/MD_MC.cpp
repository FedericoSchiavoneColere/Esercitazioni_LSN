...
int main(){ //Esercizio 4
    ...
    if(restart!=0){
        ...
        for(int iblk=1; iblk <= nblk; iblk++) //Simulation //data blocking
        {
            ...
            Averages(iblk);   //Print results for current block //l'ultima volta fara anche il calcolo dei valori finali
        }
...
void Input(void)
{
    ...
    cout << "The program perform Molecolar_Dynamic moves with uniform translations" << endl;
...
{
    double v = 0.0, w=0.0, kin=0.0; //p=pressione, per ora non la uso, dovrebbe venire circa -0.5
    double vij, wij;
    double dx, dy, dz, dr;
    ...
    for (int i=0; i<npart-1; ++i){
        ...
        // distance i-j in pbc
            ...
            if(dr < rcut){ //se è minore della distanza di cutoff faccio calcolo
                ...
                wij = 48.0*pow(1/dr,12) - 24.0*pow(1/dr,6); // viriale, al posto di 1 mettere il raggio
                //dell'argon in unità di raggio di bhor, circa 10. c'è anche epsilon ma devo capire come...
                //forse invece va bene così, ho guardato sulle slide delle lezioni...
                v += vij;
                w += wij;
            }
        }
    }
    //w/=npart; //sono indeciso se vada fatta la media o no, non credo perche è estensiva
    for (int i=0; i<npart; ++i) kin += 0.5 * (vx[i]*vx[i] + vy[i]*vy[i] + vz[i]*vz[i]);

    walker[iv] = 4.0 * v; // Potential energy //qui c'è il 4*
    walker[ik] = kin; // Kinetic energy
    walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
    walker[ie] = 4.0 * v + kin;  // Total energy;
    walker[ip] = rho*walker[it]+w/(3.0*vol); //Kb=1
...
{
    //utilizza quello calcolato nel block avareges per ffare valore medio e quadratico
    ...
    stima_kin = blk_av[ik]/blk_norm/(double)npart; //energia cinetica
    glob_av[ik] += stima_kin;
    glob_av2[ik] += stima_kin*stima_kin;
    err_kin=Error(glob_av[ik],glob_av2[ik],iblk);

    stima_etot = blk_av[ie]/blk_norm/(double)npart; //energia totoale
    glob_av[ie] += stima_etot;
    glob_av2[ie] += stima_etot*stima_etot;
    err_etot=Error(glob_av[ie],glob_av2[ie],iblk);

    stima_temp = blk_av[it]/blk_norm; //Temperatura
